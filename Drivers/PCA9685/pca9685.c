...
{
	uint8_t data[] = {value};
    return HAL_I2C_Mem_Write( handle->i2c_handle, handle->device_address, address,
                           0x1, data, 0x1, PCA9685_I2C_TIMEOUT) == HAL_OK;
}
...
static bool pca9685_write_data(pca9685_handle_t *handle, uint8_t address, uint8_t *data, size_t length)
{
    ...
    return HAL_I2C_Mem_Write( handle->i2c_handle, handle->device_address, address,
                           0x1, data, length, PCA9685_I2C_TIMEOUT) == HAL_OK;
}

static bool pca9685_read_u8(pca9685_handle_t *handle, uint8_t address, uint8_t *dest)
{
    return HAL_I2C_Mem_Read( handle->i2c_handle, handle->device_address, address,
                           0x1, dest, 0x1, PCA9685_I2C_TIMEOUT) == HAL_OK;
}
...
bool pca9685_init(pca9685_handle_t *handle)
{
	...
	// Set MODE1 registers to default values :
	// {Restart disabled, Internal Clock, Auto-Increment, LowPower, 0, 0, 0, ALLCALL}
	uint8_t mode1_reg_default_value = 0b00110001u;

	// Set MODE2 registers to default values :
	// { ..., totem pole structure, 0, 0}
	...
	success &= pca9685_set_pwm_frequency(handle, 1500);
